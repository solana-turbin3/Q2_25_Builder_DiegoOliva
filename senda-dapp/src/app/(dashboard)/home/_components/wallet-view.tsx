...
export default function SendaWallet() {
  ...
  const handleOpenTransactionDetails = (transaction: RawTransaction) => {
    ...
    // Parse signatures from deposit record
    const signatures = transaction.depositRecord?.signatures?.map((sig: string) => {
      try {
    ...
    const transactionDetails: TransactionDetailsData = {
      ...
      depositRecord: transaction.depositRecord ? {
        policy: transaction.depositRecord.policy === 'DUAL' ? 'DUAL' : 'SINGLE',
        signatures: transaction.depositRecord.signatures,
        state: transaction.depositRecord.state
      } : undefined
    };
